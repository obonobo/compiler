...
long getmemword(long addr) {
	long wordaddr;
	if (outofrange(addr) || misaligned(addr)) return 0;
	wordaddr = addr >> 2;
	if (wordaddr == mar)
		cycles += 1;
	else {
...
void putmemword(long addr, long data) {
	long wordaddr;
	if (outofrange(addr) || misaligned(addr)) return;
	wordaddr = addr >> 2;
...
void storereg(unsigned short regnum, long data) {
	if (regnum > 15) runtimeerror("simulator error (illegal register code)");
	if (regnum > 0) regs[regnum] = data;
}
...
char opnames[last][6] = {"",      "lw",   "lb",   "sw",   "sb",   "add",  "sub",  "mul",  "div",
                         "mod",   "and",  "or",   "not",  "ceq",  "cne",  "clt",  "cle",  "cgt",
                         "cge",   "addi", "subi", "muli", "divi", "modi", "andi", "ori",  "ceqi",
                         "cnei",  "clti", "clei", "cgti", "cgei", "sl",   "sr",   "getc", "putc",
                         "bz",    "bnz",  "j",    "jr",   "jl",   "jlr",  "nop",  "hlt",  "entry",
                         "align", "org",  "dw",   "db",   "res"};
...
void showfmta(long addr, wordtype word) {
	...
	switch (word.fmta.op) {
		...
		case cgt:
		case cge:
			printf("%5ld %-6s   r%d, r%d, r%d", addr, opcode, word.fmta.ri, word.fmta.rj,
			       word.fmta.rk);
			break;
		...
		case not:
		case jlr:
			printf("%5ld %-6s   r%d, r%d", addr, opcode, word.fmta.ri, word.fmta.rj);
			break;

		/* No operands */
		case nop:
		case hlt:
			printf("%5ld %-6s", addr, opcode);
			break;
	}
}
...
void showfmtb(long addr, wordtype word) {
	...
	switch (word.fmtb.op) {
		...
		case lw:
		case lb:
			printf("%5ld %-6s   r%d, %d(r%d)", addr, opcode, word.fmtb.ri, word.fmtb.k,
			       word.fmtb.rj);
			break;
		...
		case sw:
		case sb:
			printf("%5ld %-6s   %d(r%d), r%d", addr, opcode, word.fmtb.k, word.fmtb.rj,
			       word.fmtb.ri);
			break;
		...
		case cgti:
		case cgei:
			printf("%5ld %-6s   r%d, r%d, %d", addr, opcode, word.fmtb.ri, word.fmtb.rj,
			       word.fmtb.k);
			break;
		...
		case bnz:
		case jl:
			printf("%5ld %-6s   r%d, %d", addr, opcode, word.fmtb.ri, word.fmtb.k);
			break;
		...
		case ptc:
		case jr:
			printf("%5ld %-6s   r%d", addr, opcode, word.fmtb.ri);
			break;

		/* Operands K */
		case j:
			printf("%5ld %-6s   %d", addr, opcode, word.fmtb.k);
			break;
	}
}
...
void showword(long addr) {
	...
	switch (mem[wordaddr].cont) {
		...
		case 'b':
			...
			break;
		case 'd':
			printf("%5ld  %08lX  %s  %4ld", addr, word.data, wordtochars(charbuf, word), word.data);
			break;
		case 'u':
			printf("%5ld  ??", addr);
			break;
...
long getsymbolval(char *name) {
	struct symnode *p = symbols;
	while (p) {
		if (!strcmp(name, p->name)) return (p->val);
		p = p->next;
	}
	return -1;
}
	...
	switch (cont) {
		...
		case 'a':
			switch (ir.fmta.op) {
				/* add Ri, Rj, Rk */
				case add:
					storereg(ir.fmta.ri, fetchreg(ir.fmta.rj) + fetchreg(ir.fmta.rk));
					newreg = ir.fmta.ri;
					break;
				...
				case div:
					...
					if (rk == 0)
						...
						storereg(ir.fmta.ri, fetchreg(ir.fmta.rj) / rk);
						newreg = ir.fmta.ri;
					}
					break;
		...
		case 'b':
			switch (ir.fmtb.op) {
				...
				case lw:
					storereg(ir.fmtb.ri, getmemword(fetchreg(ir.fmtb.rj) + (long)ir.fmtb.k));
					newreg = ir.fmtb.ri;
					break;
				...
				case addi:
					storereg(ir.fmtb.ri, fetchreg(ir.fmtb.rj) + (long)ir.fmtb.k);
					newreg = ir.fmtb.ri;
					break;
				...
				case divi:
					...
					if (k == 0)
						...
						storereg(ir.fmtb.ri, fetchreg(ir.fmtb.rj) / k);
						newreg = ir.fmtb.ri;
					}
					break;
				...
				case sl:
					storereg(ir.fmtb.ri, fetchreg(ir.fmtb.ri) << (long)ir.fmtb.k);
					newreg = ir.fmtb.ri;
					break;
				...
				case bz:
					if (fetchreg(ir.fmtb.ri) == 0) ic = (long)ir.fmtb.k;
					break;
				...
				case ptc:
					if (tracing)
						printf("  Output from putc: %c", fetchreg(ir.fmtb.ri));
...
void showreg(short regnum) {
	...
	printf("   r%d =  %08lX  %s  %4ld", regnum, word.data, wordtochars(charbuf, word), word.data);
}
...
void traceinstr() {
	...
	if (running) {
		if (newreg >= 0)
			...
			long addr = newmem >> 2;
			printf("   M[%ld] =  %08lX  %s  %ld", newmem, mem[addr].word.data,
			       wordtochars(charbuf, mem[addr].word), mem[addr].word.data);
		}
		printf("\n");
	...
	while (1) {
		...
		if (!strcmp(cmd, "q") || !strcmp(cmd, "Q"))
			...
			switch (*cp++) {
				...
				case 'B':
					if (*cp == '\0') {
						...
						for (addr = 0; addr < MEMSIZE; addr++) {
							if (mem[addr].breakpoint) printf("%ld ", addr << 2);
						}
						printf("\n");
					} else {
						addr = getoperand(cp);
						if (addr >= 0) mem[addr >> 2].breakpoint = TRUE;
					}
					break;
				...
				case 'C':
					if (*cp == '\0') {
						for (addr = 0; addr < MEMSIZE; addr++) mem[addr].breakpoint = FALSE;
					} else {
						addr = getoperand(cp);
						if (addr >= 0) mem[addr >> 2].breakpoint = FALSE;
					}
					break;
...
enum tokentype { T_BAD, T_REG, T_OP, T_SYM, T_NUM, T_STR, T_COMMA, T_LP, T_RP, T_NULL };
...
int getint(long addr) {
	...
	if (labs(val) <= 32767) return (int)val;
...
void load(FILE *inp, FILE *out, short listing) {
	...
	while (fgets(buffer, BUFLEN - 1, inp)) {
		...
		linenum++;
		readline();
		if (listing) fprintf(out, "%5d %5ld %s", linenum, oldaddr, buffer);
		if (strcmp(errmes, "")) {
			printf("%5d %5ld %s", linenum, oldaddr, buffer);
			printf("      >>>>> %s\n", errmes);
			if (listing) fprintf(out, "      >>>>> %s\n", errmes);
		}
	}
}
...
void main(int argc, char *argv[]) {
	...
	if (listreq) {
		if (strlen(outname) == 0) strcpy(outname, "moon.prn");
	...
	for (fil = 0; fil < numfiles; fil++) {
		if (!strchr(filedescs[fil].name, '.')) strcat(filedescs[fil].name, ".m");
